...
function find_gene_start(xs) {
    ...
    return is_null(xs)
        ? null
        : head(xs) === "A"
        ? (!is_null(tail(xs)) && head(tail(xs)) === "T"
            ? (!is_null(tail(tail(xs))) && head(tail(tail(xs))) === "G"
...
function find_gene_end(xs) {
    ...
    // WRITE HERE.
    function helper(ys, count) {
        return is_null(ys)
            ? null
            : head(ys) === "T"
            ? (!is_null(tail(ys)) && (head(tail(ys)) === "A" || head(tail(ys)) === "G")
                ? (!is_null(tail(tail(ys))) && (head(tail(tail(ys))) === "G" || head(tail(tail(ys))) === "A")
                    ? (count === 0
                        ? list(null)
                        : count)
                    : helper(tail(ys), count + 1))
                : helper(tail(ys), count + 1))
            : helper(tail(ys), count + 1);
    }
    ...
    if (is_number(helper(xs, 0))) {
        for (let i = helper(xs, 0) - 1; i >= 0; i = i - 1) {
            lst = pair(list_ref(xs, i), lst);
        }
...
function all_genes(xs) {
    ...
    // WRITE HERE.
    function helper(ys, zs) {
        return is_null(ys)
            ? zs
            : helper(find_gene_start(head(ys)), append(zs, find_gene_end(head(ys))));
    }
    return helper(find_gene_start(xs), null);
}

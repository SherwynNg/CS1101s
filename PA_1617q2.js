...
function all_different(nums) {
    ...
    for (let i = 0; i < len; i = i + 1) {
        if (i === len - 1) {
            ...
            for (let k = i + 1; k < len; k = k + 1) {
                if (list_ref(nums, i) === list_ref(nums, k)) {
                    return false;
                }
            }
        }
...
function is_valid_toto_set(nums, n, min, max) {
    ...
    // WRITE HERE.
    const len = length(nums);
    let min1 = list_ref(nums, 0);
    let max1 = list_ref(nums, 0);
    if (len === n) {
        for (let i = 1; i < len; i = i + 1) {
            if (list_ref(nums, i) < min1) {
                min1 = list_ref(nums, i);
            }
            
            for (let k = 1; k < len; k = k + 1) {
                if (list_ref(nums, k) > max1) {
                    max1 = list_ref(nums, k);
                }
            }
        }
...
function num_of_matches(numsA, numsB) {
    ...
    // WRITE HERE.
    let matches = 0;
    const lenA = length(numsA);
    const lenB = length(numsB);
    if (lenA < lenB) {
        for (let i = 0; i < lenA; i = i + 1) {
            for (let k = 0; k < lenB; k = k + 1) {
                if (list_ref(numsA, i) === list_ref(numsB, k)) {
                    matches = matches + 1;
                }
            }
        }
        ...
        for (let i = 0; i < lenB; i = i + 1) {
            for (let k = 0; k < lenA; k = k + 1) {
                if (list_ref(numsA, i) === list_ref(numsB, k)) {
                    matches = matches + 1;
                }
            }
...
function check_winning_group(bet_nums, draw_nums, extra_num) {
    ...
    // WRITE HERE.
    function no_matches(bet_nums, draw_nums) {
        let matches = 0;
        const lenA = length(bet_nums);
        const lenB = length(draw_nums);
        for (let i = 0; i < lenA; i = i + 1) {
            for (let k = 0; k < lenB; k = k + 1) {
                if (list_ref(bet_nums, i) === list_ref(draw_nums, k)) {
                    matches = matches + 1;
                }
            }
        }
    ...
    function extra_match(extra_num, ys) {
        return is_null(ys)
            ? false
            : head(ys) === extra_num
            ? true
            : extra_match(extra_num, tail(ys));
    }
    
    const number_of_matches = no_matches(bet_nums, draw_nums);
    const match_extra = extra_match(extra_num, bet_nums);
    const n = length(draw_nums);
    if (number_of_matches === n) {
        return 1;
    } else if (number_of_matches === n - 1 && match_extra) {
        return 2;
    } else if (number_of_matches === n - 1) {
        return 3;
    } else if (number_of_matches === n - 2 && match_extra) {
        return 4;
    } else if (number_of_matches === n - 2) {
        return 5;
    } else {
        return 0;
    }
